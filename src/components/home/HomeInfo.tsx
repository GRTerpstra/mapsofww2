...
const HomeInfo = () => {
  ...
  var documentCardNumber = 1;

  /**
   * Increases the document card number so that the next document gets displayed.
   */
  function incrementDocumentCardNumber() {
    documentCardNumber == 3 ? documentCardNumber = 1 : documentCardNumber++;
    var input = document.getElementById('card' + documentCardNumber) as HTMLInputElement;
    input.checked = true;
  }

   /**
   * Decreases the document card number so that the previous document gets displayed.
   */
  function decrementDocumentCardNumber() {
    documentCardNumber == 1 ? documentCardNumber = 3 : documentCardNumber--
    var input = document.getElementById('card' + documentCardNumber) as HTMLInputElement;
    input.checked = true;
  }
  ...
  return (
    <div className='home-info'>
      ...
      <div className="home-info__content">
        <ArrowRightIcon className="home-info__arrow home-info__arrow-previous" onClick={() => decrementDocumentCardNumber()} />
        ...
        <ArrowRightIcon className="home-info__arrow home-info__arrow-next" onClick={() => incrementDocumentCardNumber()} />
      </div>
    </div>
  )
}
